...
CountryStruct = SelfAwareStruct.new(:me, :populations, :executive, :head, :office, :legislatures)
...
module Query
  class CountryInfo
    ...
    def data
      h = results.first
      CountryStruct.new(h[:country], [h[:population]], *h.values_at(:executive, :head, :office), legislatures)
    end
